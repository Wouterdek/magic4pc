...
const globals = {
	__DEV__: true,
	ILIB_BASE_PATH:
		iLibDirs.find((f) => fs.existsSync(path.join(app.context, f))) ||
		iLibDirs[1],
...
for (let t = app.theme; t; t = t.theme) {
	...
	globals[rbConst(t.name)] = path
		.relative(app.context, themeRB)
		.replace(/\\/g, '/');
}
...
module.exports = {
	...
	setupFilesAfterEnv: [require.resolve('./setupTests'), userSetupFile].filter(
		Boolean
	),
	testMatch: [
	...
	moduleNameMapper: {
		...
		'^@testing-library/react-hooks$': require.resolve(
			'@testing-library/react-hooks'
		),
		'^@testing-library/user-event$': require.resolve(
		...
		'^@enact[/]i18n[/]ilib[/](.*)$': path.join(
			app.context,
			globals.ILIB_BASE_PATH,
			'$1'
		),
	},
